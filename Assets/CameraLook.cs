...
public class CameraLook : MonoBehaviour
{
    ...
    void MoveCamera()
    {
        //It looks like when unity get my mouse position, if my mouse is under the play button it will be count as a giant rotate. 
        //Therefore this is my solution that I let it get those values after a short time after the scene is loaded.
        if (Time.timeSinceLevelLoad > 1)
        {
            //get the mouse position from unity
            float mouseX = Input.GetAxis("Mouse X");
            float mouseY = Input.GetAxis("Mouse Y");

            //rotation value = position * senstivity value * deltatime 
            rotationY += mouseX * mouseSenstivity * Time.deltaTime;
            rotationX += mouseY * mouseSenstivity * Time.deltaTime;

            //Limit the rotation to prevent extreme rotate
